...
export default function eyeDonor() {
  ...
  async function eyeBankRequest() {

    try {

      const token = await AsyncStorage.getItem('token');
      const request = await axios.post(`${baseUrl}/eyeBankRequest`, { token });
  ...
  async function approve(item: any) {
    try {
      const token = await AsyncStorage.getItem('token');
      const request = await axios.post(`${baseUrl}/eyeBankRequestApprove`, { token, requestId: item.id, status: item.status });
      console.log(request.data.message)
      if (request.status === 200) {
        setNotFound(request.data.message)
        setDiplayRequestBox(false)
  ...
  return (
    ...
    >
      ...
      <View style={style.box2}>
        ...
        <TouchableOpacity style={style.box2Btn} onPress={eyeBankRequest}>
