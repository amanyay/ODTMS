...
export default function kidneyDonorCompleteTransplant() {

  const [request, setRequest] = useState<any>([]);
  const [notFound, setNotFound] = useState('');
  const [diplayRequestBox, setDiplayRequestBox] = useState(false)

  async function eyeBankTransplantComplete() {

    try {

      const token = await AsyncStorage.getItem('token');
      const request = await axios.post(`${baseUrl}/kidneyTransplantComplete`, { token });

      console.log(request.data.message)
      if (request.status === 200) {
        setDiplayRequestBox(true);
        setRequest(request.data.message)
      }
  ...
  async function approve(item: any) {
    try {
      const token = await AsyncStorage.getItem('token');
      const request = await axios.post(`${baseUrl}/kidneyTransplantCompleteApprove`, { token, requestId: item.id, status: item.status  , rec_phone_number : item.rec_phone_number , 
         don_phone_number : item.don_phone_number
      });
      if (request.status === 200) {
        setNotFound(request.data.message)
        setDiplayRequestBox(false)
  ...
  useEffect(() => { eyeBankTransplantComplete() }, [])
  ...
  return (
    ...
    >
      ...
      <View style={style.box2}>
        ...
        <TouchableOpacity style={style.box2Btn} onPress={eyeBankTransplantComplete}>
            ...
            <View style={style.organBox}>
              ...
              <View style={style.organBox1}>
                ...
                <View style={style.boxHeaderText}>
                  <Text style={style.boxHeaderText1}>On Going Oprations</Text>
