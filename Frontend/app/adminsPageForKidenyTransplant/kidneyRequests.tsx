...
export default function eyeDonor() {
  ...
  async function kidneyRequestData() {

    try {

      const token = await AsyncStorage.getItem('token');
      const request = await axios.post(`${baseUrl}/kidneyRequest`, { token });
  ...
  async function approve(item: any) {
    try {
      const token = await AsyncStorage.getItem('token');
      const request = await axios.post(`${baseUrl}/kidneyRequestApprove`, { token, requestId: item.id, status: item.status });
      if (request.status === 200) {
        setNotFound(request.data.message)
        setDiplayRequestBox(false)
      }
  ...
  return (
    ...
    >
      ...
      <View style={style.box2}>
        ...
        <TouchableOpacity style={style.box2Btn} onPress={kidneyRequestData}>
