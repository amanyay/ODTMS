...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const selectionFromWaitingListTable = await adminModel.selectionFromWaitingListTable(recipentsPhoneNumber, donorPhoneNumber)
        const selectionFromWaitingListTableToCheckDuplicateDonor = await adminModel.selectionFromWaitingListTableToCheckDuplicateDonor(donorPhoneNumber)
        const selectionFromWaitingListTableToCheckDuplicateRecipents = await adminModel.selectionFromWaitingListTableToCheckDuplicateRecipents(recipentsPhoneNumber)
        ...
        if (selectionFromWaitingListTableToCheckDuplicateRecipents.length === 0 && selectionFromWaitingListTableToCheckDuplicateRecipents.length === 0) {
            if (selectionFromWaitingListTable.length === 0) {
                ...
                const insertionIntoWaitingList = await adminModel.insertionIntoWaitingList(recipentsPhoneNumber, donorPhoneNumber, organId)
